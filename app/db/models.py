...
class Asset(Base):
    ...
    # Portfolio columns (added in migration 0003)
    exchange = Column(String(20))
    yf_symbol = Column(String(20))           # symbol as passed to yfinance (e.g. ATZ.TO)
    quantity = Column(Numeric(16, 6))
    market_price = Column(Numeric(16, 4))
    market_price_currency = Column(String(10), server_default="CAD")
    book_value_cad = Column(Numeric(16, 4))
    market_value_cad = Column(Numeric(16, 4))
    unrealized_return_cad = Column(Numeric(16, 4))
    created_at = Column(
        DateTime(timezone=True), nullable=False, server_default=func.now()
    )
